...
function staticRequest(pathname) {
  return new Promise((resolve, reject) => {
    const request = http.get({ host: '127.0.0.1', port: STATIC_PORT, path: pathname }, response => {
      const chunks = [];
      response.on('data', chunk => chunks.push(chunk));
      response.on('end', () => {
        try { resolve({ status: response.statusCode, value: JSON.parse(Buffer.concat(chunks)) }); }
...
async function requireOwnerData() {
  const status = await staticRequest('/game-data/status');
  if (status.status !== 200 || !status.value.ready) {
...
server.listen(PUBLIC_PORT, '0.0.0.0', () => {
  process.stdout.write(`quake3-wasm: framework 0.7.0 + QuakeJS supervisor on tcp/${PUBLIC_PORT}\n`);
});
